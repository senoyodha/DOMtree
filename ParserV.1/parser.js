                    ...
                    while (true) {
                        ...
                        while (true) {
                            ...
                            if (posTemp != -1)
                                ...
                                node = stackOpen[removedNode[0] - 1];
                                removedNode = [];
                            }
                            if (node == formatEl)
                                break;
                        ...
                        while (true) {
                            insertNode(newEl, removeNode(furthestBlock.firstChild), 'last');
                            if (furthestBlock.firstChild.next == null)
                                break;
                        }
                                    ...
                                    case "DOCTYPE":
                                        ...
                                        var flagStart = function () {
                                            ...
                                            for (var i in arrayPublic)
                                                if (token[2].substr(0, arrayPublic[i].length).toUpperCase() == arrayPublic[i].toUpperCase())
                                                    return true;
                                            var arrayPublic = ["-//W3C//DTD HTML 4.01 Frameset//", "-//W3C//DTD HTML 4.01 Transitional//"];
                                            for (var i in arrayPublic)
                                                if (token[3] == null && token[2].substr(0, arrayPublic[i].length).toUpperCase() == arrayPublic[i].toUpperCase())
                                                    return true;
                                            return false;
                                        };
                                        ...
                                        var flagQuirks = token[4] || newNode.name != "html" || arrayPublic.indexOf(token[2].toUpperCase()) != -1 || token[3].toUpperCase() == "http://www.ibm.com/data/dtd/v11/ibmxhtml1-transitional.dtd".toUpperCase() || flagStart();
                                ...
                                switch (token[0]) {
                                    ...
                                    case "StartTag":
                                        if (token[1] == "html")
                                            ...
                                            insertNode(adjustInsert.mode == 'last' ? adjustInsert.target : adjustInsert.target.parent, newNode, adjustInsert.mode);
                                            stackOpen.push(newNode);
                                ...
                                switch (token[0]) {
                                    ...
                                    case "StartTag":
                                        if (token[1] == "html") {
                                            ...
                                            if (hasButtonScope("p"))
                                                closePElement();
                                            if (["h1", "h2", "h3", "h4", "h5", "h6"].indexOf(currentNode().type) != -1) {
                                                emit("ParseError");
                                                stackOpen.pop();
                                            }
                                            insertHTMLElement(token);
                                            ...
                                            for (var i in listFormat)
                                                if (listFormat[listFormat.length - i - 1].type == "marker") {
                                                    lastMarker = listFormat.length - i - 1;
                                                    break;
                                                }
                                            var flag2 = -1;
                                            for (var i = lastMarker; i < listFormat.length; i++)
                                            ...
                                            var posTemp = stackOpen.indexOf(listFormat[flag2]);
                                            var nodeTemp = [listFormat[flag2], stackOpen[posTemp]];
                                            emit("ParseError");
                                            var resAdopt = adoptionAgency('a', token[4], token);
                                            if (resAdopt == -1)
                                                flag = true;
                                            else {
                                                if (listFormat[flag2] == nodeTemp[0]) {
                                                    if (stackOpen[posTemp] == nodeTemp[1])
                                                        stackOpen.splice(posTemp, 1);
                                                    listFormat.splice(flag2, 1);
                                                }
                                            }
                                            ...
                                            if (["rtc", "ruby"].indexOf(currentNode().type) == -1)
                                                emit("ParseError");
                                            insertHTMLElement(token);
                                        }
                                            ...
                                            insertForeignElement(token, nsEl['mathml']);
                                            if (token[3]) {
                                                stackOpen.pop();
                                                acknowledgeSelfClose(token);
                                            }
                                            ...
                                            adjustSVGAttr(token);
                                            adjustForeignAttr(token);
                                            insertForeignElement(token, nsEl['svg']);
                                            if (token[3]) {
                                                stackOpen.pop();
                                                acknowledgeSelfClose(token);
                                            }
                            ...
                            case 'In table': //12.2.5.4.9 The "in table" insertion mode
                                ...
                                switch (token[0]) {
                                    ...
                                    case "Comment":
                                        ...
                                        break;
                                    case "Character":
                                        if (["table", "tbody", "tfoot", "thead", "tr"].indexOf(currentNode().type) != -1) {
                                            pendingCharTable = [];
