...
function preprocessing(stream) {
    try {
        ...
        for (var i = 0; i < stream.length; i++) {
            ...
            else if (0xD800 <= stream.codePointAt(i) && stream.codePointAt(i) <= 0xDFFF) {
                logs.push('\t\tParse error: Surrogate character "' + String.fromCharCode(stream.codePointAt(i)) + '" (U+' +
                    stream.codePointAt(i).toString(16).toUpperCase() + ')');
                parseError++;
            }
            if (stream.codePointAt(i) > 0xFFFF)
                i++;
        }
        if (parseError == 0)
            logs.push('\t\tNo control or surrogate characters');
