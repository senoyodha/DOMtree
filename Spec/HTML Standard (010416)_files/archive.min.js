'use strict';(function($){var TILE_WIDTH=180; // NOTE: needs to stay in sync w/ archive.less
...
if(navHT===false){for(var i in styles){styles[i]=$(i).attribute('style');}}var tmp=$('#nav-tophat').css({'position':'fixed','width':'100%','z-index':10000001,'top':0}).outerHeight();if(navHT===false)navHT=tmp;$('.navbar').css({'position':'fixed','width':'100%','z-index':10000000,'top':0,'padding-top':navHT});}).on('shown.bs.collapse',function(){ // "sm" width size *can* make some nav tophat text re-flow to 2 lines and push height down...
...
for(var i in styles){$(i).attribute('style',typeof styles[i]=='undefined'?'':styles[i]);}});}$('#nav-tophat').on('show.bs.collapse',function(){$('#navwrap1').addClass('hatted');}).on('hide.bs.collapse',function(){$('#navwrap1').removeClass('hatted');});$('.navbar a.navia-link').on('click',function(evt){ // are we in "mobile nav / xs width" mode?
var xs=$('#nav-tophat-helper:visible').length?false:true;var hat_open=$('#nav-tophat').hasClass('in');var navlink=this;var mt=$(evt.target).attribute('data-top-kind'); //user hit text
if(!mt)mt=$(evt.currentTarget).attribute('data-top-kind'); //user hit icon
...
if(typeof archive_analytics!="undefined"){archive_analytics.send_ping({'service':'ao_2','kind':'event','ec':'page_action','ea':'nav_tophat_galactica','el':location.pathname,'cache_bust':Math.random()});}}}var isnow=$('.navbar .dropdown-ia.hatted a').attribute('data-top-kind');if(typeof isnow!='undefined')log('isnow '+isnow);log('shouldB '+mt);if(isnow==mt){ // 2nd click on same mediatype dropdown -- close tophat!
...
if($e.attribute('id')=='nav-wb-url')return; // ... except clicking on wayback machine form input
...
if(!AJS.wrapdark_watcher_ptr)AJS.wrapdark_watcher_ptr=setInterval(AJS.wrapdark_watcher,1000);if(!AJS.$sharedown.length||!AJS.$wrapdark.length){clearInterval(AJS.wrapdark_watcher_ptr);return;}var wrapdarkHT=AJS.$sharedown.offset().top+AJS.$sharedown.find('.panel-heading').innerHeight();if(parseInt(AJS.$wrapdark.css('height'),10)!=wrapdarkHT){AJS.$wrapdark.css({height:wrapdarkHT}).show();log('       WRAPDARK-ED to '+wrapdarkHT);}};AJS.flash_click=function(flashy){if(flashy)$.cookie('avpref2',null,{path:'/'});else $.cookie('avpref2','flash',{path:'/'});window.top.location.href=location.href;return false;};AJS.mute_click=function(){var mess=typeof JSMESS!='undefined';var dos=typeof DOSBOX!='undefined';var mutedNOW=!$.cookie('unmute');if(mess){if(!(JSMESS&&JSMESS.sdl_pauseaudio||typeof _SDL_PauseAudio!='undefined')){alert('This button only works once the emulation is running');return false;}if(mutedNOW){JSMESS.sdl_pauseaudio&&JSMESS.sdl_pauseaudio(0)||_SDL_PauseAudio&&_SDL_PauseAudio(0);}else {JSMESS.sdl_pauseaudio&&JSMESS.sdl_pauseaudio(1)||_SDL_PauseAudio&&_SDL_PauseAudio(1);}alert('Page requires reload for un/mute to take effect');}else if(dos){if(mutedNOW){typeof _SDL_PauseAudio!='undefined'&&_SDL_PauseAudio(0);}else {typeof _SDL_PauseAudio!='undefined'&&_SDL_PauseAudio(1);}alert('Page requires reload for un/mute to take effect');}else {var player=jwplayer('jw6');if(player){var volnow=player.getVolume();if(volnow!=0){AJS.mute_click_prior_volume=volnow;player.setVolume(0);}else {player.setVolume(typeof AJS.mute_click_prior_volume=='undefined'?100:AJS.mute_click_prior_volume);}}}$('#theatre-ia .iconochive-mute, #theatre-ia .iconochive-unmute').toggle();if(mutedNOW){ // sounds is off.  make it loud
...
var right_gutter_width=Math.round(($('#theatre-ia-wrap').width()-$('#theatre-controls').width()-width)/2);log('width: '+width);log('right_gutter_width: '+right_gutter_width);$('#theatre-controls').css({height:height,visibility:'visible',right:Math.max(20,right_gutter_width/2)});};AJS.booksize=function(){if(!AJS.booksize_watching){AJS.booksize_watching=true;$(window).on('resize  orientationchange',function(evt){$.doTimeout('AJS.booksize'+evt.type,250,AJS.booksize);});}var aspect=4/3.0;var metadataHeight=100; //metadata peekaboo min height!
var maxH=$(window).height()-$('#texty').offset().top-metadataHeight;var maxW=$('#texty').width();log('max rect avail: '+maxW+'x'+maxH);var vidW,vidH;var tries=[960,840,720,600,480,360,240,180];for(var idx in tries){vidH=tries[idx];vidW=Math.round(vidH*aspect);log('rect size try fit: '+vidW+'x'+vidH);if(vidW<=maxW&&vidH<=maxH||vidW<=320)break;} // We want to vertically "center" books that are "very short" that
// would otherwise be shorter than the new "minimum height" requirements
// for the theatre
var padTop=0;var minH=parseInt($('#theatre-ia-wrap').css('min-height'),10);if(minH>vidH){padTop=Math.round((minH-vidH)/2);}$("#texty iframe").css({width:vidW,height:vidH,'padding-top':padTop}); // resize bookreader
...
AJS.booksize_controls=function(){var $iframe=$('iframe:first');if(!$iframe.length)return;var $iframeDOM=$($iframe.get(0).contentDocument);if(!$iframeDOM.length)return;if(!AJS.booksize_controls_hidden){if(!$iframeDOM.find('#BRnav').length){log('BOOK NOT READY YET');return;}$iframeDOM.find('#BRnav').hide();AJS.booksize_controls_hidden=true;$("#texty iframe").css('visibility','visible');log('BOOK CONTROLS HIDDEN!');}$iframeDOM.find('body, #BRcontainer').css('background-color','transparent'); // now position the theatre controls
var $book=$iframeDOM.find('#BRtwopageview'); // find out how much vertical black padding the bookreader put in *OVER* the book,
// inside the iframe
...
$('#theatre-ia').css({'overflow':'hidden'});};AJS.popcornsize=function(){$(window).on('load  resize  orientationchange',function(evt){$.doTimeout('AJS.popcorn_sizer'+evt.type,250,function(){var metadataHeight=100; //metadata peekaboo min height!
...
});});};AJS.pause=function(id){if(!id)id='jw6';var jw=jwplayer(id);if(jw&&jw.config&&jw.getState()=='PLAYING')jw.pause();};AJS.colplay_setup=function(){$('#tabby-collection .row .item-ia:not(.collection-ia) .item-img').parent().tooltip({'title':'<span class="iconochive-play"></span> Click image to Preview','placement':'bottom','html':true});$('#tabby-collection .row .item-ia:not(.collection-ia)').on('click',function(evt){var identifier=$(evt.target).parents('a').attribute('href').replace(/\/details\//,'');$.get('/details/'+identifier+'&colplay=1',function(data){$('body').addClass('colplay');setTimeout(function(){$('.container-ia:first').css({'padding-top':60+$('.welcome').height()});AJS.tiler();},1000);if($('#colplay').length){AJS.pause();}else {$('.container-ia:first').append('<div id="colplay"/>');}try{$('#colplay').html('<div class="colplay-exit" data-toggle="tooltip" data-placement="left" title="hide player preview"><span class="iconochive-play"></span></div>'+data);$('#colplay .colplay-exit').tooltip({}).click(function(){AJS.pause();$('#colplay').css({'right':'-50%'});setTimeout(function(){$('#colplay').remove();$('body').removeClass('colplay');$('.container-ia:first').css({'padding-top':''});AJS.tiler();},600);});$('#colplay .sharee[data-toggle="tooltip"]').tooltip({});$('#colplay').css({'right':0});}catch(e){log('********* EXCEPTION ******');log(e);}});return false;});};AJS.add2list=function(lnk){var selector=$(lnk).attribute('data-target');var href=$(lnk).attribute('href');var selectorID=selector.replace(/#/,'');$.get(href+'&headless=1&titlelist='+$(lnk).text(),function(htm){ // dynamically add modal to page (if isnt there already)
AJS.modal_go(lnk,{title:'Add To List',headerClass:'modal-header-new-list',ignore_lnk:true,auto_remove:true,body:htm});});return false;};AJS.newlist=function(lnk){var selector=$(lnk).attribute('data-target');var href=$(lnk).attribute('href');var selectorID=selector.replace(/#/,'');$.get(href+'&headless=1',function(htm){ // dynamically add modal to page (if isnt there already)
...
//   #ikind-search              (search)
//
AJS.selector=function(){var inTab=$('.welcome .tabby.in .tabby-text').text();if(inTab=='Forum'||inTab=='Posts'||inTab=='About')return false; // no tab of tiles are showing
//alert(inTab);
var selector=false;if(inTab){ // We want to end up with a selector like:
//    #ikind-collections-title
//    #ikind--publicdate
log('inTab: '+inTab);var tmp='#tabby-'+inTab.toLowerCase();if(inTab=='Collection'||inTab=='Collections'||inTab=='Uploads'){selector='#'+$(tmp+' div.ikind.in:first').attribute('id');}else {var ikind=$(tmp+' .ikind.stealth.in:first').text().toLowerCase().replace(/ /,'-');selector='#ikind-'+inTab.toLowerCase()+(ikind===""?"":"-"+ikind);}}else { // we want to end up with a selector like:
//    #ikind-date
...
}if(!selector)selector=AJS.selector();if(selector===false)return;var $selector=$(selector).first();var selectorID=$selector.attribute('id');var $parentRow=$selector.parents('.row');if(!$('body').hasClass('tiles')){$parentRow.css({'padding-left':'','padding-right':''});return;}if(!AJS.tilerPREV){ // first time tiling page!
...
// TestLib place new tile *there* (in "first cycled slot") UNLESS further ahead in cycle/ring
...
var tmp=$val.find('.item-img');if(tmp.length&&!$val.find('.item-img').height()){nImgZeroHeight++;tmp=$val.attribute('data-id');if(tmp)nImgZeroHeightStr+=tmp+',';} // setup for next tile...
...
var $sel=$('#'+id).parents('.tabby-data').find('select.ikind-mobile');var $new=$sel.find('option:contains('+$(lnk).text()+')');var $now=$sel.find('option:selected');if($new.text()!=$now.text()){log('changing ikind mobile now to '+$new.text());AJS.ikind_mobile_change_ignore_next=true;$new.attribute('selected','selected'); //$now.removeAttr('selected');
...
var $ikind=$ikinds.filter(function(index){return $(this).text()===$selopt.text();});var href=$ikind.attribute('href');log('goto: '+href);if(href){if(href.substr(0,1)=='#'){ // ACCOUNT PAGE (FOR NOW)!  (STILL USES HASH FOR NOW)
...
var $item=$(xer).parents('.item-ia');var idX=$item.attribute('data-id');var url=location.href.replace(/#.*$/,'')+'&remove_item='+idX; // Use the most recent update date to help uniquely identify the item!
if($item.attribute('data-date'))url+='&data-date='+$item.attribute('data-date');if(AJS.listkind)url+='&kind='+AJS.listkind;log('GET: '+url);$.get(url,function(htm){log('GOT: '+url);log(htm);$item.remove();AJS.tiler();});}return false;}; // remove an element from a search results page
...
var ids=$(".item-ia[data-id]").toArray().reduce(function(a,b){var id=$(b).attribute('data-id');return ""+a+(id=="__mobile_header__"?"":id+",");},"");log('MANAGE: '+ids);if(ids!==""){ // now make a form that we can POST to, with all the identifiers,
...
AJS.grafs=function(){var $grafs1=$('#grafs1');var $grafs2=$('#grafs2');if(!$grafs1.length)return;if(typeof $grafs1.attribute('data-id')=='undefined')return;var identifier=$grafs1.attribute('data-id');$grafs1.attribute('data-id',null);log('loading grafs');$grafs1.html('<i><small>loading graph <img src="/images/loading.gif"/></small></i>').show();$grafs2.html('<i><small>loading graph <img src="/images/loading.gif"/></small></i>').show();$.get('/details/'+identifier+'&grafs=1&v=2',function(htm){var tok="<h2>";var a=htm.split(tok);if(a.length>=3){$('#activity-reviewsN').html(a[1]).parents('.activity-box').show();$('#activity-forumN').html(a[2]).parents('.activity-box').show();}if(a.length==5){$grafs1.html(tok+a[3]);$grafs2.html(tok+a[4]);}else {$grafs1.html('(graph data not available)');$grafs2.html('(graph data not available)');}});};AJS.quick_down=function(id,target){if(!$('.format-group.in').length){ // no set of files for a single format showing... yet!
...
$('.nav-hamburger .navbar-nav').clone().removeClass('nav navbar-nav').attribute('id','nav-abouts')); // both of the above divs are hidden.  now fade them in
...
if(tumble){var seen='';$('.tumbles').each(function(k,v){seen+=$(v).attribute('data-idx')+",";});url+='&seen='+seen;}log('more_search(selector='+selector+', sort='+ikind+', page='+page+', url='+url+')'); //log(AJS.page_map);
...
AJS.more_searching=false;return;}var selectorID=$(selector).attribute('id');if(AJS.tilerPREV&&AJS.tilerPREV.unsourced&&AJS.tilerPREV.unsourced[selectorID])delete AJS.tilerPREV.unsourced[selectorID];if(tumble){$('.results').append(htm);$more_search.find('.more-search-fetching').hide();AJS.more_searching=false;return;}if(AJS.pushState&&typeof history.replaceState!='undefined'){history.replaceState({},'',urlreplace);}var $selector=$(selector+' .results');htm=AJS.addNotes(AJS.addingNotesKind,htm);$selector.append(htm); // re-tile and re-flow!  (the force should flow through you)
...
$('body').prepend($('<div/>').attribute({'id':embid+'Shadow','class':'textarea-invert-readonly roundbox5'}).css({'position':'absolute','visibility':'hidden','top':60,'left':10,'padding':'5px 15px 5px 15px','width':$embid.width(),'font-size':$embid.css('font-size')}).text(embtxt));var bestHT=$('#'+embid+'Shadow').outerHeight()+15;log(embid+' bestie height: '+bestHT+' (for current width: '+$embid.width()+')');$('#'+embid+'Shadow').remove();$embid.height(bestHT);}}; // dynamically adds modal to page (if isnt there already)
...
$.cookie('beta-modal','open',{path:'/',expires:1,domain:'.archive.org'});var selector=$(lnk).attribute('data-target');conf.href=location.href;conf.href=conf.href.replace(/&ui3=1$/,'');conf.href=conf.href.replace(/\/v2$/,'');conf.body='\n\
...
}var selector=$(lnk).attribute('data-target');var href=$(lnk).attribute('href');AJS._modal_add(selector,conf);if(conf.shown){$(selector).on('shown.bs.modal',function(evt){conf.shown();});}if(conf.follow_link_on_dismiss){$(selector).on('hidden.bs.modal',function(evt){log('modal hidden, going to '+href+'..');$('body').removeClass('blurry');location.href=href;});}$(selector).modal('show');if(conf.login&&$.cookie('logged-in-user')===null){if(location.protocol!='https:'){ // make absolutely sure we never login with http!
...
if(conf.auto_close&&conf.ignore_lnk)setTimeout(function(){$(selector).modal('hide');},2000);return false;}; // (adapted) from http://www.queness.com/code-snippet/6853/center-an-element-on-the-screen-using-jquery
...
var avgVal=Math.round($(dominantColorRGB).map(function(idx,val){return val;}).toArray().reduce(function(a,b){return a+b;})/dominantColorRGB.length);var color=avgVal>=128?'black':'white';log('avgVal: '+avgVal);$('.welcome').css({'color':color,'background-color':'rgb('+dominantColorRGB[0]+','+dominantColorRGB[1]+','+dominantColorRGB[2]+')'});$('.welcome .stealth:not(.tabby .stealth)').css({'color':color});}; /**
   ...
   */AJS.makeMouseElementAccessible=function(element){$(element).attribute("tabindex","0").on("keypress keydown",function(e){if(e.type!="click"){var code=e.keyCode||e.which;var k_space=32; // space bar pressed
...
}};AJS.editNote=function(e){var $e=$(e);var identifier=$e.parents('.details-ia').prev().attribute('data-id');if(!identifier)return false;var $note=$e.parents('.note');var val='';if($e.text()=='edit'){val=$note.find('span:first').text();val=val.replace(/</g,'&lt;').replace(/>/g,'&gt;');}$note.hide();var $append=$note.parent();$append.append($('\
...
'));return false;};AJS.editedNote=function(e){var $e=$(e);var identifier=$e.parents('.details-ia').prev().attribute('data-id');if(!identifier)return; // if user hit [return] and submitted form, etc.
...
var htm='';$(AJS.thumbzillas).each(function(key,val){var start=parseInt(val,10);if(TV){var $end=start+TV3.CLIP_SEC_MAX2;var hash='#start/'+start+'/end/'+$end;htm+='<a onclick="$(\'#opscreen1M\').fadeOut(\'slow\'); TV2.seekHash(\''+hash+'\')" href="#start/'+start+'/end/'+$end+'">';}else {htm+='<a onclick="$(\'#opscreen1M\').fadeOut(\'slow\'); Play(\'jw6\').seek('+start+'); return false" href="&start='+start+'">';}htm+='<img src="/download/'+id+'/'+id+'.thumbs/'+id+'_'+val+'.jpg"/></a>';});if(htm===''){alert('coming soon!');return false;}$('#opscreen1M').remove();$('body').prepend($('<div />').attribute({'id':'opscreen1M'}));var $imgs=$('<div/>').attribute({'id':'thumbzilla'}).html('<div>'+htm+'</div>');$imgs.appendTo('#opscreen1M');$('#opscreen1M').css({'visibility':'hidden'}).show();var winH=$(window).height()+(AJS.ios?60:0); // plus iOS bleah
 ...
 */(function($){var a={},c="doTimeout",d=Array.prototype.slice;$[c]=function(){return b.apply(window,[0].concat(d.call(arguments)));};$.fn[c]=function(){var f=d.call(arguments),e=b.apply(this,[c+f[0]].concat(f));return typeof f[0]==="number"||typeof f[1]==="number"?this:e;};function b(l){var m=this,h,k={},g=l?$.fn:$,n=arguments,i=4,f=n[1],j=n[2],p=n[3];if(typeof f!=="string"){i--;f=l=0;j=n[1];p=n[2];}if(l){h=m.eq(0);h.data(l,k=h.data(l)||{});}else {if(f){k=a[f]||(a[f]={});}}k.id&&clearTimeout(k.id);delete k.id;function e(){if(l){h.removeData(l);}else {if(f){delete a[f];}}}function o(){k.id=setTimeout(function(){k.fn();},j);}if(p){k.fn=function(q){if(typeof p==="string"){p=g[p];}p.apply(m,d.call(n,i))===true&&!q?o():e();};o();}else {if(k.fn){j===undefined?e():k.fn(j===false);return true;}else {e();}}}})(jQuery);(function($){$(function(){AJS.nav_tophat_setup(); // when DOM loaded/stable, do some setup
$(function(){if(typeof archive_setup=='function')archive_setup();if(typeof archive_setup_search=='function')archive_setup_search();if(typeof archive_setup_collection=='function')archive_setup_collection();});AJS.footer();});})(jQuery);
