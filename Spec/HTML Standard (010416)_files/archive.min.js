...
//    #ikind--publicdate
log('inTab: '+inTab);var tmp='#tabby-'+inTab.toLowerCase();if(inTab=='Collection'||inTab=='Collections'||inTab=='Uploads'){selector='#'+$(tmp+' div.ikind.in:first').attr('id');}else {var ikind=$(tmp+' .ikind.stealth.in:first').text().toLowerCase().replace(/ /,'-');selector='#ikind-'+inTab.toLowerCase()+(ikind===""?"":"-"+ikind);}}else { // we want to end up with a selector like:
//    #ikind-date
...
if(!AJS.tilerPREV.unsourced[selectorID]){log('unsourcing '+selectorID);$selector.find('.item-ia img[source]').each(function(k,v){v=$(v);v.attr({src:v.attr('source')}).removeAttr('source');});AJS.tilerPREV.unsourced[selectorID]=1;} // we track where previous tile was placed
